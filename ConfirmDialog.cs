...
public class ConfirmDialog : IDialog
{
    ...
    public bool Confirmed => IsComplete && !WasCancelled;
    ...
    {
        if (KeyPressed(keyboard, prevKeyboard, Keys.Y))
        {
            IsComplete = true;
            WasCancelled = false;
        }
    ...
    public void Draw(SpriteBatch spriteBatch, SpriteFont font, Renderer renderer, int screenWidth, int screenHeight, GameTime gameTime)
    {
        ...
        string hint = "[Y] Yes    [N] No";
        var hintSize = font.MeasureString(hint);
        spriteBatch.DrawString(font, hint, new Vector2(px + PanelWidth - hintSize.X - 10, py + PanelHeight - font.LineSpacing - 8), HintColor);
    }
